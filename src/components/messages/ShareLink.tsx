...
const ShareLink = ({ qid }: { qid?: string | undefined }) => {
  ...
  return (
    <>
      <div
        ...
        onClick={() => {
          const linkToCopy = qid
            ? `${bURL}/${cookies?.user?.username}?q=${qid}`
            : `${bURL}/${cookies?.user?.username}`;
      ...
      >
        ...
        <span className="flex p-2 items-center justify-between border-[0.75px] rounded-lg text-ellipsis cursor-pointer border-primary">
          <p className="text-primary/80 hover:text-primary duration-700 font-[0.9rem]" suppressHydrationWarning>
            {qid
              ? `${bURL}/${cookies?.user?.username}?q=${qid}`
              : `${bURL}/${cookies?.user?.username}`}
          </p>
